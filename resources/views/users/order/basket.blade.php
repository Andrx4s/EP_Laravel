                ...
                <div class="border border-1 rounded-3 p-3">
                    ...
                    <table class="table">
                        ...
                        @if(isset($products))
                            ...
                            <tr>
                                <td><img width="50px" height="50px" alt="" src="/public/storage/{{$product->photo}}"></td>
                                <td>{{$product->name}}</td>
                                <td>{{$product->price}}</td>
                                <td><input type="number" name="productsIds[{{$product->id}}]" value="{{session('basket')[$product->id]}}" class="form-control"></td>
                            ...
                            <form action="{{route('order.createOrder')}}" method="POST">
                                ...
                                <tr>
                                    <td colspan="5">
                                        <p class="small">Если данные товара не сохранены, то будет создан заказ с предыдущим указанным количеством товаров!</p>
                                        <label for="inputAddress" class="form-label">Адрес доставки</label>
                                            <input type="text" id="inputAddress" class="form-control mb-2" name="address" value="{{Auth::user()->address}}">
